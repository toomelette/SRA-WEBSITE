    ...
    <div class="container-fluid">
        <div class="row">
          <div class="col-lg-9 posts-list">
            {{ Breadcrumbs::render('bid_suppBid') }}
            <div class="single-post row">

              <div class="col-lg-12 col-md-12 blog_details">
                <h3 class="text-heading title_color">Supplemental Bid</h3>
              </div>
              
              <div class="container">

                <form data-pjax class="form" id="filter_form" method="GET" autocomplete="off" action="{{ route('guest.bid.supp_bid') }}">
                ...
                <div id="pjax-container">
                  <table class="table table-striped table-bordered" style="width:100%">
                    ...
                    <tbody>
                      @foreach ($supp_bid_list as $data)
                        <tr>
                          <td id="mid-vert">{{ $data->description }}</td>
                          <td id="mid-vert">{{ $data->station }}</td>
                          <td id="mid-vert">{{ __dataType::date_parse($data->date, 'm/d/Y') }}</td>
                          <td id="mid-vert">
                            <a href="{{ route('guest.bid.view_supp_bid', $data->slug) }}" class="genric-btn btn-info small" target="_blank">
                              Download
                            </a>
                          </td>
                        </tr>
                  ...
                  @if($supp_bid_list->isEmpty())
                    <div style="padding :5px;">
                      <center><h4>No Records found!</h4></center>
                    </div>
                  @endif

                  {!! __html::table_counter($supp_bid_list) !!}
                  {!! $supp_bid_list->appends([])->render('vendor.pagination.bootstrap-4')!!}
